...
function statusBadge(status: string): string {
  const map: Record<string, string> = {
    ...
    interrupted: 'bg-gray-100 text-gray-400',
...
function StepOutput({ output }: { output: string | null }) {
  ...
  if (!output) return <span className="text-xs text-gray-400 italic">No output</span>;
...
function RunRow({ run }: { run: Run }) {
  ...
  const hasPendingGate = run.steps.some(s => s.gateStatus === 'pending' || s.gateStatus === 'revision_requested');
    ...
    <div>
      <div
        ...
        className={`bg-white rounded-lg border p-4 cursor-pointer hover:shadow-sm transition-shadow ${
          hasPendingGate ? 'border-amber-300 border-l-4' : 'border-gray-200'
        }`}
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="text-sm font-medium text-gray-800 truncate">{run.workflowName}</div>
              <div className="text-xs text-gray-400">{run.id.slice(0, 8)}</div>
            </div>
          </div>
          <div className="flex items-center gap-4 flex-shrink-0">
            ...
            <span className="text-xs text-gray-400 w-16 text-right">{relativeTime(run.startedAt)}</span>
      ...
      {expanded && (
        <div className="bg-gray-50 border border-t-0 border-gray-200 rounded-b-lg ml-4">
          {run.steps.length === 0 ? (
            <div className="p-4 text-xs text-gray-400 italic">No steps recorded</div>
          ) : (
...
function WorkflowPage() {
  ...
  const { data: runsData, isLoading } = useQuery({
    ...
    queryFn: () => fetch(`${API}/api/v1/workflows/runs`).then(async r => {
      // Fetch full run details (with steps) for each run
      const runs = await r.json();
      const detailed = await Promise.all(
        runs.map((run: any) =>
          fetch(`${API}/api/v1/workflows/runs/${run.id}`).then(r => r.json())
        )
      );
      return detailed;
    }),
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Trigger section */}
      <div>
        <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Available Workflows</h3>
        {workflowList.length === 0 ? (
          <p className="text-gray-400 text-sm">No workflows configured</p>
        ) : (
          <div className="grid gap-3 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {workflowList.map((wf: any) => (
              <div key={wf.name} className="bg-white rounded-lg border border-gray-200 p-4 flex flex-col justify-between">
                <div>
                  <div className="text-sm font-medium text-gray-800">{wf.name}</div>
                  <div className="text-xs text-gray-500 mt-1">{wf.description || 'No description'}</div>
                  <div className="text-xs text-gray-400 mt-2">{wf.steps} steps</div>
                </div>
                <button
      ...
      <div>
        <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">
          Recent Runs
          {runs.length > 0 && <span className="ml-2 text-gray-400 font-normal normal-case">{runs.length}</span>}
        </h3>
        {isLoading ? (
          ...
          <p className="text-gray-400 text-sm py-4">No workflow runs yet</p>
        ) : (
          <div className="space-y-2">
            {runs.map((run: Run) => (
